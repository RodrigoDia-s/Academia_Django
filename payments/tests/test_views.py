...
from pedidos.tests.factories import planoFactory
...
def client_with_order(client, order_form_data):
    p = planoFactory(price=1.0)
    client.post(
        reverse("cart:add", kwargs={"plano_id": p.id}),
        data={"quantity": 1, "override": False},
    )
    client.post(reverse("pedidos:create"), order_form_data, follow=True)
    return client
...
def test_payment_webhook(order, client, plano, mocker):
